...
static ConfigParsingCodes_t parseJsonConfig( const cJSON *cfgJson, WaterscreenConfig_t *config )
{
    if ( cfgJson == NULL )
...
ConfigParsingCodes_t fromJsonToWaterscreenCfg( const char *cfgJson, WaterscreenConfig_t *config )
{
    ...
    const ConfigParsingCodes_t parsingCode = parseJsonConfig( configJson, config );
