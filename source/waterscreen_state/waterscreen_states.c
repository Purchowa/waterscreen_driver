...
static inline uint16_t lastElementIndex(const pictureData_t *picture) { return (picture->rowCount - 1); }
...
void presentationState(WaterscreenContext_t *context)
{
	...
	{
		changeWaterscreenState(context, demoModeState); // TODO: go back to previous state. Not necessarily demoMode
